...
void genCodeFromASTree(ASTNode* node, int count, FILE *out)
{
  ...
  switch (node->type) {
   ...
   case AST_WHILE:
      ...
      fprintf(out, "LL%d:\n", y);
      genCodeFromASTree(node->child[0],x,out);  // child 0 is condition expr
      break;
   case AST_IFTHEN:
      // fprintf(out,"If then\n");
      x = getUniqueLabelID();
   ...
   case AST_RELEXPR:
       ...
       switch (node->ival) {
         case '<': assem = "jl"; break;
         case '>': assem = "jg"; break;
         case '!': assem = "jne"; break;
         case '=': assem = "je"; break;
         default: assem = "unknown relop";
       }
       fprintf(out,"\t%s\tLL%d\n",assem,count);
       break;
   case AST_VARREF:
      fprintf(out,"\n\tmovl\t%s (%%rip),\t%%eax",node->strval);
      if (node->varKind == V_GLARRAY) {
         fprintf(out," array ref\n");
